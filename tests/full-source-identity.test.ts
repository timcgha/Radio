...
describe("authoritative source-pin precedence", () => {
  it("short project/display SHA cannot downgrade full ObjectiveAuthority expectedStartingSha", () => {
    expect(
      resolveAuthoritativeExpectedBaseTipSha({
        objectiveAuthorityExpectedStartingSha: LEVEL3_FULL,
        solClaimedExpectedStartingSha: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
  ...
  it("Sol claim is ignored when trusted ObjectiveAuthority pin is present", () => {
    expect(
      resolveAuthoritativeExpectedBaseTipSha({
        objectiveAuthorityExpectedStartingSha: LEVEL3_FULL,
        expectedStartingSha: "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
        transactionBranchTipSha: LEVEL3_SHORT,
        fallbackFullSha: "aa512d6ef721f855be33ddc36da490f9de66dc23",
      }),
    ).toBe(LEVEL3_FULL);
  ...
  it("latest live shape: abbreviated PROJECT-STATE mainSha cannot override ObjectiveAuthority pin", () => {
    const baseline = acceptedBaselineState();
    expect(baseline.canonicalState.mainSha).toBe(LEVEL3_SHORT);

    // Simulate stale short tip still present on transaction state.
  ...
  it("objective-start stores full ObjectiveAuthority SHA on transaction; work order uses trusted pin", () => {
    const dir = tmpDir();
    ...
    expect(prepared.state.currentTransaction?.sourceBaseTipSha).toBe(LEVEL3_FULL);
    expect(prepared.state.currentTransaction?.branch).toBe("level3");
...
describe("mocked live source-ref precheck (latest live shape)", () => {
  async function transmitLive(input: {
    ...
    // Work order always grounds tip in trusted ObjectiveAuthority. For remote
    // precheck regressions that need a non-authority tip, force after build.
    let { workOrder } = buildStage3WorkOrder({
      state: prepared.state,
      decision,
    });
    expect(workOrder.source.expectedBaseTipSha).toBe(LEVEL3_FULL);
    if (input.expectedBaseTipSha !== LEVEL3_FULL) {
      workOrder = {
        ...workOrder,
        source: {
          ...workOrder.source,
          expectedBaseTipSha: input.expectedBaseTipSha,
        },
      };
    }
